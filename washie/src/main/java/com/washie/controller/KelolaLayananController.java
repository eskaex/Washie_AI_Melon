...
public class KelolaLayananController implements Initializable {
    ...
    private void hapusLayanan(Layanan l) {
        ...
        c.showAndWait().filter(b -> b == ButtonType.YES).ifPresent(b -> { layananService.hapus(l.getIdLayanan()); loadLayanan(); });
    }
    ...
    private void loadAddon() {
        ...
        for (Layanan a : addons) {
            ...
            row.setStyle("-fx-background-color:#FFFFFF; -fx-background-radius:8; " + "-fx-border-color:#D0D8E0; -fx-border-radius:8; " + "-fx-border-width:1;");
    ...
    private void hapusAddon(Layanan a) {
        ...
        c.setTitle("Konfirmasi");
        c.showAndWait().filter(b -> b == ButtonType.YES).ifPresent(b -> layananService.hapus(a.getIdLayanan()));
    }
